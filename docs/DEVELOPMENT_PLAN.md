...
#### 태스크 목록
- [ ] **1.1. 환경 변수 및 의존성 정비**
  - `.env.local`에 `NEXT_PUBLIC_GEMINI_API_KEY` 설정 가이드 추가
  - `@google/generative-ai` 패키지 설치 또는 공식 fetch REST 인터페이스 구축
- [ ] **1.2. 기존 서버 API Route 제거/마이그레이션**
  - PRD 제약 1에 따라 `app/api/summarize/route.ts`, `app/api/quiz/route.ts` 제거
  - 클라이언트 사이드 전용 AI 호출 헬퍼(`lib/gemini-client.ts`) 신규 작성
- [ ] **1.3. PDF 전처리 및 클라이언트 사전 검증 로직 구현 (제약 4)**
  - 브라우저 `File` 객체들을 `inlineData`(Base64) 파트로 변환하는 비동기 유틸 작성
  - 파일 총합 10MB 초과 검사 로직 구현
  - 텍스트 추출 시 15,000자 초과 검사 로직 구현
  - 검증 실패 시 API 호출 사전 차단 및 한국어 에러 처리

#### 완료 기준 (DoD)
- Next.js 서버 라우트를 거치지 않고 브라우저에서 Gemini 1.5 Flash 모델로 PDF 데이터가 전송되는 테스트 완료
...
- [ ] **2.3. 고유 ID 기반 퀴즈 상태 관리 및 채점 시스템 (제약 3)**
  - 답안 State를 `Record<number, number>` (Key: 문제 `id`, Value: 선택한 보기 0~3)로 설계
  - `[문제 섞어 다시 풀기]` 기능 구현:
    - `id` 기준 답안 상태 완벽 리셋 (`setUserAnswers({})`, `setIsGraded(false)`)
    - 문항 배열 셔플 (Fisher-Yates 알고리즘 적용)
  - `[새로운 문제 생성]` 파이프라인 연동
- [ ] **2.4. 미풀이 문항 채점 예외 처리**
...
#### 태스크 목록
- [ ] **3.1. `@media print` 전용 CSS 구현 (제약 2)**
...
- [ ] **4.2. 필수 기술적 제약 사항 4가지 코드 주석화**
  - 소스코드 내 `[기술적 제약 사항 1~4]` 반영 위치에 명확한 설명 주석 기재
- [ ] **4.3. 빌드 및 배포 검증**
  - `pnpm build`를 통한 TypeScript 및 린트 정적 검사 통과
  - Vercel 배포 시 서버리스 페이로드 에러(4.5MB/10s timeout)가 발생하지 않음을 검증

#### 완료 기준 (DoD)
- 모든 예외 케이스 및 PRD 요구사항 체크리스트 100% 통과
- `pnpm build` 무결성 확인 및 README/개발 문서 최신화

---

## 4. 제약 사항별 코드 구현 매핑 체크리스트

| 제약 번호 | 제약 명칭 | 소스코드 반영 위치 (예정) | 구현 내용 |
| :--- | :--- | :--- | :--- |
| **제약 1** | 백엔드/서버리스 금지 | `lib/gemini-client.ts`, `components/pdf-study-app.tsx` | Next.js API Routes 제거, 클라이언트에서 직접 `@google/generative-ai` 호출 |
| **제약 2** | 네이티브 인쇄 / `@media print` | `app/globals.css`, `components/pdf-study-app.tsx` | `window.print()` 호출, `@media print`로 UI 제어 및 A4 규격 스타일링 |
| **제약 3** | 고유 ID 기반 퀴즈 채점 & 셔플 리셋 | `components/pdf-study-app.tsx` | `q.id`를 키로 답안 저장, 셔플 시 `setUserAnswers({})` 초기화 |
| **제약 4** | 용량 및 텍스트량 사전 차단 | `lib/pdf-validator.ts` or `lib/gemini-client.ts` | 파일 10MB / 텍스트 15,000자 초과 검증 후 throw 및 Alert 차단 |

---

## 5. 진척도 및 개발 상태 트래킹

- **Sprint 1 (기반 아키텍처 & 클라이언트 AI)**: ⏳ 준비 중
- **Sprint 2 (핵심 모드 & 고유 ID 퀴즈)**: ⏳ 대기
- **Sprint 3 (인쇄 스타일 & UI/UX)**: ⏳ 대기
- **Sprint 4 (예외 처리 & 최종 검증)**: ⏳ 대기
